...
class MultiScan(nn.Module):
    ...
    def multi_scan(self, x):
        ...
        # 第一部分的通道
        x_first_group = x[:, :channels_per_group, :, :]
        # 第二部分的通道
        x_second_group = x[:, channels_per_group:, :, :]
        # xs.append(self.scan(x_first_group, 'h'))
        # xs.append(self.scan(x_first_group, 'h_flip'))
        # xs.append(self.scan(x_second_group, 'w2'))
        # xs.append(self.scan(x_second_group, 'w2_flip'))

        h_scan = self.scan(x_first_group, 'h')  # 执行 'h' 扫描
        w2_scan = self.scan(x_second_group, 'w2')  # 执行 'w2' 扫描
        h_flip_scan = self.scan(x_first_group, 'h_flip')  # 执行 'h_flip' 扫描
        w2_flip_scan = self.scan(x_second_group, 'w2_flip')  # 执行 'w2_flip' 扫描

        # 1. 将 'h' 和 'w2' 的结果进行 concat
        concat_h_w2 = torch.cat((h_scan, w2_scan), dim=1)  # 沿着通道维度 (dim=1) 进行拼接

        # 2. 将 'h_flip' 和 'w2_flip' 的结果进行 concat
        concat_h_flip_w2_flip = torch.cat((h_flip_scan, w2_flip_scan), dim=1)  # 沿着通道维度拼接

        # 3. 将新的拼接结果存储到 xs 中
        xs = []  # 或者清空/初始化 xs
        xs.append(concat_h_w2)  # 添加 'h' 和 'w2' 的拼接结果
        xs.append(concat_h_flip_w2_flip)  # 添加 'h_flip' 和 'w2_flip' 的拼接结果
    ...
    def multi_reverse(self, xs):
        ...
        # 对 xs[0] 进行分割并分别进行 'h' 和 'w2' 转换
        x_part1, x_part2 = torch.chunk(xs[0], 2, dim=1)  # 沿着通道维度分成两部分
        x_part1_h = self.reverse(x_part1, 'h')  # 对第一部分执行 'h' 转换
        x_part2_w2 = self.reverse(x_part2, 'w2')  # 对第二部分执行 'w2' 转换
        concat_h_w2 = torch.cat((x_part1_h, x_part2_w2), dim=1)  # 沿着通道维度拼接

        # 将拼接结果添加到 new_xs
        new_xs.append(concat_h_w2)

        # 对 xs[1] 进行分割并分别进行 'h_flip' 和 'w2_flip' 转换
        x_part1_flip, x_part2_flip = torch.chunk(xs[1], 2, dim=1)  # 分成两部分
        x_part1_h_flip = self.reverse(x_part1_flip, 'h_flip')  # 对第一部分执行 'h_flip' 转换
        x_part2_w2_flip = self.reverse(x_part2_flip, 'w2_flip')  # 对第二部分执行 'w2_flip' 转换
        concat_h_flip_w2_flip = torch.cat((x_part1_h_flip, x_part2_w2_flip), dim=1)  # 拼接

        # 将拼接结果添加到 new_xs
        new_xs.append(concat_h_flip_w2_flip)

        # 处理 xs 中的其他张量（如果有更多）
        for x, direction in zip(xs[2:], self.choices[2:]):  # 跳过前两个
            new_xs.append(self.reverse(x, direction))
...
class MultiMamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_middle_cls_token:
            # step 3
            xs = [torch.cat([x[:, :, :cls_position], cls_token.transpose(-2, -1), x[:, :, cls_position:]], dim=2) for x
                  in xs]
