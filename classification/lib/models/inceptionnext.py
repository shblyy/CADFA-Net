...
class MetaNeXt(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # print("num_classes123", num_classes)
        num_stage = len(depths)
        if not isinstance(token_mixers, (list, tuple)):
            token_mixers = [token_mixers] * num_stage
        if not isinstance(mlp_ratios, (list, tuple)):
        ...
        for i in range(num_stage):
            ...
            stages.append(MetaNeXtStage(
                prev_chs,
                out_chs,
                ds_stride=2 if num_classes == 2 else 1,
                # ds_stride=2,
                depth=depths[i],
                drop_path_rates=dp_rates[i],
                ls_init_value=ls_init_value,
    ...
    def forward(self, x):
        ...
        # 通过 stem 提取初步特征
        x = self.stem(x)
        features[0] = x  # 存储 stem 的输出特征

        # 遍历模型的 stage
        for idx, layer in enumerate(self.stages):
            x = layer(x)
            features[0] = x  # 因为只有一个 stage，特征图直接存储在 features[0] 中

        return features  # 返回最终的特征图
        # scale = [4, 8, 16, 32]  #下采样倍数
        # scale = [1, 2, 4, 8]
        # features = [None, None, None, None] #初始化一个列表，用来存储不同尺度的特征图
        # # print("stem前",x.shape)
