...
from lib.models.cs.ema import EMA
...
class CAB(nn.Module):
    def __init__(self, num_feat, is_light_sr=False, compress_ratio=3, squeeze_factor=30):
        ...
        # nn.Conv2d(num_feat, num_feat // compress_ratio, kernel_size=1),  # 降维
            #
            # nn.Conv2d(num_feat // compress_ratio, num_feat // compress_ratio, kernel_size=3, stride=1, padding=1),
            # # 3x3 卷积
            # nn.GELU(),
            # nn.Conv2d(num_feat // compress_ratio, num_feat, kernel_size=1),  # 升维

            # nn.Conv2d(num_feat, num_feat, 3, 1, 1),
            # 深度可分离卷积
            # nn.Conv2d(num_feat, num_feat // compress_ratio, kernel_size=3, stride=1, padding=1,
            #           groups=num_feat // compress_ratio),
            # nn.Conv2d(num_feat // compress_ratio, num_feat // compress_ratio, kernel_size=1),
            # nn.GELU(),
...
#对基类MultiScan扩展的一个实现，引入双向注意力机制，对输入特征图进行多方向扫描和融合
class MultiScanVSSM(MultiScan):
    ALL_CHOICES = MultiScan.ALL_CHOICES #可供扫描的方向
    #choices可能代表不同扫描方向，BiAttn双向注意力
    def __init__(self, dim, choices=None, num_groups=2):
        super().__init__(dim, choices=choices, token_size=None)
        self.attn = BiAttn(dim)
        self.num_groups = num_groups  # 默认将通道分成两组

    #特征融合
    def merge(self, xs,xs1):
        # xs: [B, K, D, L]
        # return: [B, D, L]
    #b代表batch,K是方向或扫描次数，D通道数，L序列长度,代表从不同方向扫描后的特征图
        ...
        for i in range(len(xs1)):
            # 检查是否存在 `xs1[i][0]`，然后将其添加到 `xs` 列表中
            if len(xs1[i]) > 0:  # 确保 `xs1[i][0]` 存在
                xs2.append(xs1[i][1])
        ...
        # print(xs[1].shape)
        # print("-------------------------")
        # xs2 = [xs2[:, i, :, :l] for i, l in enumerate(self.scan_lengths)]
        # print("xs:", xs.shape)
        xs = super().multi_reverse(xs,xs2)
        # print("xs3:", xs[0].shape)
...
#不同方向进行扫描
    def multi_scan(self, x):
        ...
        #保存输入特征图的空间尺寸
        self.token_size = (H, W)
#从多个方向（super() 中定义的扫描方式）对输入特征图进行扫描，输出一个列表 xs
        # print(x.shape)
        xs1 = super().multi_scan(x)  # [[B, C, H, W], ...]torch.Size([4, 1536, 49])两个
        # xs=[]
        # xs.append(xs1[0][0])
        # xs.append(xs1[1][0])
        # xs.append(xs1[2][0])
        ...
        #记录每个扫描结果的长度，x.shape[2] 表示扫描后的序列长度。
        self.scan_lengths = [x.shape[2] for x in xs]
#填充至相同长度：为了使不同方向的扫描结果能进行合并，先找到最长的扫描结果 max_length，对较短的扫描结果进行 F.pad() 操作
        max_length = max(self.scan_lengths)
#过 torch.stack(new_xs, 1) 将所有方向的特征图堆叠在一起，形成形状为 [B, K, C, H*W] 的张量，其中𝐾
#K 是扫描方向的数量。
        ...
        for x in xs:
            ...
            new_xs.append(x)
        # print(torch.stack(new_xs, 1).shape)
        return torch.stack(new_xs, 1),xs1

    def __repr__(self):
        scans = ', '.join(self.choices)
        #其中将类名 MultiScanVSSM 替换为 MultiScanVSSM[scans]，scans 是当前选择的扫描方式列表。
...
def multi_selective_scan(
    ...
    L = H * W
    #确定的是局部窗口的行数，如果没有指定，将根据通道数 D 来选择合适的值。
    if nrows < 1:
        if D % 4 == 0:
            nrows = 4
        elif D % 3 == 0:
            ...
            nrows = 1
    #多方向扫描操作
    xs,xs1 = multi_scan.multi_scan(x)
    L = xs.shape[-1]
    #投影操作，将扫描后的特征与投影权重x_proj_weight进行张量乘法，得到投影结果
    x_dbl = torch.einsum("b k d l, k c d -> b k c l", xs, x_proj_weight)  # l fixed
    #如果 x_proj_bias 存在，使用 view 对其进行调整，然后与投影后的张量 x_dbl 相加。
    if x_proj_bias is not None:
        x_dbl = x_dbl + x_proj_bias.view(1, K, -1, 1)
    #沿着通道维度（dim=2）拆分成 dts、Bs 和 Cs 三部分。
    ...
    #特征融合，将扫描结果 ys 通过 multi_scan.merge 方法进行融合，这里应该是将不同扫描方向的结果合并为一个张量。随后通过归一化层 out_norm 进行正则化。
    y = multi_scan.merge(ys,xs1)
    #最终输出的结果 y 会转换回输入张量的原始数据类型 x.dtype，并返回
...
def local_vssm_tiny(*args, drop_path_rate=0.2, **kwargs):
    ...
    return VSSM(dims=[96, 192, 384, 768], depths=[2, 2, 9, 2], d_state=16, drop_path_rate=drop_path_rate,
                directions=directions)
...
def local_vssm_small(*args, drop_path_rate=0.2, **kwargs):
    ...
    return VSSM(dims=[96, 192, 384, 768], depths=[2, 2, 27, 2], d_state=16, drop_path_rate=drop_path_rate,
                directions=directions)
