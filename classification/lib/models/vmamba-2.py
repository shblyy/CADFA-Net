...
from lib.models.inceptionnext import MetaNeXt
from lib.models.inceptionnext import MetaNeXt1
...
class Backbone_LocalVSSM(VSSM):
    def __init__(self, out_indices=(0, 1, 2, 3), pretrained=None, norm_layer=nn.LayerNorm, **kwargs):
        ...
        #-----------------新增修改----------------------
        self.meta_next_backbone = MetaNeXt(
            # in_chans=48,  # 输入通道数
            num_classes=1000,  # 类别数（这里可以不重要，因为我们主要用于特征提取）
            depths=(3,),
            dims=(48,),
            pretrained=None,  # 可以设置为预训练模型路径，或者None表示没有预训练
        )
        self.meta_next_backbone1 = MetaNeXt1(
            # in_chans=48,  # 输入通道数
            num_classes=1000,  # 类别数（这里可以不重要，因为我们主要用于特征提取）
            depths=(3,),
            dims=(96,),
        ...
        self.meta_next_backbone2 = MetaNeXt1(
            # in_chans=48,  # 输入通道数
            num_classes=1000,  # 类别数（这里可以不重要，因为我们主要用于特征提取）
            depths=(9,),
            dims=(192,),
        ...
        self.meta_next_backbone3 = MetaNeXt1(
            # in_chans=48,  # 输入通道数
            num_classes=1000,  # 类别数（这里可以不重要，因为我们主要用于特征提取）
            depths=(3,),
            dims=(384,),
        ...
        self.patch_embed1 = nn.Sequential(
            nn.Conv2d(3, 96, kernel_size=4, stride=4, bias=True),
        )
        # -----------------新增----------------------
...
#---------------------------------------------------------------------------------------#
    def forward(self, x):
        def layer_forward(l, x ,res_x,x1,res1):

            x = l.blocks(x)
            dim = x.size(3)
            # print(dim)
            x = x.permute(0, 3, 1, 2).contiguous()  # 将维度从 [b, h, w, c] 变为 [b, c, h, w]
            # print("shapex:",x.shape)
            # print("shapex1:", x1.shape)
            self.meta_next_backbone.depths=(3,)
            device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
            # if dim>48:
            #     # norm_layer = nn.BatchNorm2d(int(dim/2)).to(device)
            #     print(int(dim/2))
            #     self.downsample = nn.Sequential(
            #         nn.BatchNorm2d(int(dim/2)),
            #         nn.Conv2d(int(dim/2), dim, kernel_size=2, stride=2),
            #     )
            #     self.downsample = self.downsample.to(device)

                # 将输入数据 x1 移动到 GPU（或 CPU）
                # x1 = x1.to(device)
                # x1=self.downsample(x1)
                # print("downsample:", x1.shape)
            if dim==48:
                self.meta_next_backbone.dims = (dim,)
                x1=self.meta_next_backbone(x1)[0]
            if dim == 96:
                x1 = self.meta_next_backbone1(x1)[0]
            if dim == 192:
                x1 = self.meta_next_backbone2(x1)[0]
            if dim == 384:
                x1 = self.meta_next_backbone3(x1)[0]
            # print("shapex1:", x1[0].shape)
            # print("shapex11:", x1[1].shape)
            concat_output = torch.cat([x1[0],x], dim=1)
            # print("concat_output shape:", concat_output.shape)


            # self.bn_layer = nn.BatchNorm2d(concat_output.size(1)).to(device)
            # normalized_output = self.bn_layer(concat_output)
            # print("normalized_output",normalized_output.shape)
            # print("res",res1.shape)
            # print("x1[1]",x1[1].shape)
            res_x = torch.cat([x1[1], res1], dim=1)
            #这个res_x就是所谓的弄好的特征图
            res_x = concat_output + res_x
            # print("res:",res_x.shape)
            x1, x2 = torch.chunk(res_x, 2, dim=1)
            x = x2

            x = x.permute(0, 2, 3, 1)  # 将 (B, C, H, W) 改为 (B, H, W, C)
            # y=x
            y = l.downsample(x)
            res1 = y.permute(0, 3, 1, 2).contiguous()
            # print("shapey:", y.shape)
            return x, y ,res_x,x1,res1

        x = self.patch_embed1(x)
        res_x=x
        x1, x2 = torch.chunk(x, 2, dim=1)
        # C1, C2, C3, C4 = self.meta_next_backbone(x1)
        # print(C1.shape)
        # print(C2.shape)
        # print(C3.shape)
        # print(C4.shape)
        ...
        for i, layer in enumerate(self.layers):
            ...
            o, x,res_x,x1,res1 = layer_forward(layer, x,res_x,x1,res1 ) # (B, H, W, C)
            if i in self.out_indices:
                ...
                #-------------新增
                # concat_output = torch.cat([x1, out], dim=1)
                # total_channels = concat_output.size(1)
                #
                # # 随机生成一个通道维度上的排列顺序
                # shuffled_indices = torch.randperm(total_channels)
                #
                # # 根据生成的随机顺序重新排列通道
                # out = concat_output[:, shuffled_indices, :, :]
                # print("out1:",out.shape)
                #out这里第一次输出[2, 96, 56, 56]
                #第二次输出([2, 192, 28, 28])
                #第三次输出([2, 384, 14, 14])
                #第四次输出([2, 768, 7, 7])
                # -------------
        ...
        # # 分别将 C1-C4 和 M1-M4 进行 concatenate 和 shuffle
        # concat_C1_M1 = torch.cat([C1, M1], dim=1)  # 在通道维度上拼接
        # concat_C2_M2 = torch.cat([C2, M2], dim=1)
        # concat_C3_M3 = torch.cat([C3, M3], dim=1)
        # concat_C4_M4 = torch.cat([C4, M4], dim=1)
        #
        #
        # # shuffled_C1_M1 = shuffle_channels(concat_C1_M1)
        # # shuffled_C2_M2 = shuffle_channels(concat_C2_M2)
        # # shuffled_C3_M3 = shuffle_channels(concat_C3_M3)
        # # shuffled_C4_M4 = shuffle_channels(concat_C4_M4)
        # shuffled_C1_M1 = concat_C1_M1
        # shuffled_C2_M2 = concat_C2_M2
        # shuffled_C3_M3 = concat_C3_M3
        # shuffled_C4_M4 = concat_C4_M4
        #
        # # 打印打乱后的形状
        # # print("Shuffled shapes:")
        # # print(shuffled_C1_M1.shape)
        # # print(shuffled_C2_M2.shape)
        # # print(shuffled_C3_M3.shape)
        # # print(shuffled_C4_M4.shape)
        # shuffled=[]
        # shuffled.append(shuffled_C1_M1)
        # shuffled.append(shuffled_C2_M2)
        # shuffled.append(shuffled_C3_M3)
        # shuffled.append(shuffled_C4_M4)
...
def local_vssm_tiny_search(*args, drop_path_rate=0.1, **kwargs):
    return VSSM(dims=[32, 64, 128, 256], depths=[3, 3, 9, 3], d_state=16, drop_path_rate=drop_path_rate)
