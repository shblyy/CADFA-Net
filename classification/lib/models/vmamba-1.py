...
from lib.models.inceptionnext import MetaNeXtBackbone
...
class Backbone_LocalVSSM(VSSM):
    def __init__(self, out_indices=(0, 1, 2, 3), pretrained=None, norm_layer=nn.LayerNorm, **kwargs):
        ...
        # -----------------新增修改----------------------
        self.meta_next_backbone = MetaNeXtBackbone(
            in_chans=48,  # 输入通道数
    ...
    def forward(self, x):
        ...
        # 打乱通道维度
        def shuffle_channels(concat_output):
            total_channels = concat_output.size(1)
            shuffled_indices = torch.randperm(total_channels)  # 生成随机的通道顺序
            shuffled_output = concat_output[:, shuffled_indices, :, :]  # 打乱通道顺序
