...
# from .snake_scan import snake_scan_h,snake_scan_h_flip,snake_scan_v,snake_scan_v_flip
from .snake_scan import snake_scan_with_cache
from .block_scan import snake_scan_blocks_with_cache
class MultiScan(nn.Module):
    ...
    def multi_reverse(self, xs,xs2=[1,2]):
        # print(len(xs2[1]))
        new_xs = []
        i = 0
        for x, direction in zip(xs, self.choices):
            # print(xs2[i])
            new_xs.append(self.reverse(x, direction,xs2[i]))
            i=i+1
        return new_xs
    ...
    def scan(self, x, direction='h'):
        ...
        if len(x.shape) == 3:
            ...
            if direction == 'h':
                ...
                # print(3)
                xflatten = snake_scan_with_cache(x,'v')
                return xflatten
            elif direction == 'v_flip':
                # print(4)
                xflatten = snake_scan_with_cache(x,'v_flip')
                return xflatten
            elif direction.startswith('w'):
                # print(5)
                K = int(direction[1:].split('_')[0])
                flip = direction.endswith('flip')
                # print("正向2", local_scan_bchw(x, K, H, W, flip=flip)[0, 0])

                # return local_scan_bchw(x, K, H, W, flip=flip)
                return snake_scan_blocks_with_cache(x, K, flip=flip)
    ...
    def reverse(self, x, direction='h',xs2=[1,2]):
        ...
        if direction == 'h':
            ...
            # 使用 xs2 作为索引对 xs 进行重新排序
            # 注意这里我们需要扩展 xs2_tensor 的维度以匹配 xs 的形状
            # 然后沿第三个维度进行重新排序
            x = x[:, :, xs2_tensor]
            return x
        elif direction == 'h_flip':
            ...
            # K = int(direction[1:].split('_')[0])
            # flip = direction.endswith('flip')
            # # print("反向2", local_reverse(x, K, H, W, flip=flip)[0, 0])
